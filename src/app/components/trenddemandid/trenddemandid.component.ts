...
@Component({
  ...
  templateUrl: './trenddemandid.component.html',
  styleUrls: ['./trenddemandid.component.css']
})
export class TrenddemandidComponent implements OnInit {
  ...
  ngOnInit(): void 
  {
    console.log("trenddemandid"); 
      this.userService.get_trenddemandid().subscribe((res)=>
    {
       
      for (var index1 in res) {
        console.log(res[index1]);
        this.data.push(res[index1].count)
        this.label.push(res[index1].clientname)
